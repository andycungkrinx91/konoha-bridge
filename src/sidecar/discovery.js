...
const SIDECAR_BINARY_NAMES = {
  win32: ['language_server_windows_x64.exe', 'language_server_windows_arm64.exe', 'language_server_windows.exe'],
  darwin: [
  ...
  linux: ['language_server_linux_x64', 'language_server_linux_arm64', 'language_server_linux'],
};
 ...
 *   'x:/code/marcodiniz/konoha-bridge' → 'file_x_3A_code_marcodiniz_ag_local_bridge'
 *
...
function rankProcessCandidate(proc, currentWorkspaceId) {
  ...
  let score = 0;
  if (
    proc.commandLine.includes('/resources/app/extensions/antigravity/bin/') ||
    proc.commandLine.includes('\\resources\\app\\extensions\\antigravity\\bin\\') ||
    proc.commandLine.includes('antigravity/bin/') ||
    proc.commandLine.includes('antigravity\\bin\\')
  ) {
    score += 100;
  }
  ...
  // Strongly prefer Antigravity IDE processes over agy CLI processes
  if (
    proc.commandLine.includes('--app_data_dir antigravity-ide') ||
    proc.commandLine.includes('--subclient_type ide')
  ) {
    score += 300;
  }
  if (proc.commandLine.includes('antigravity-cli') || proc.commandLine.includes('--subclient_type cli')) {
    score -= 200;
  }
...
async function execWithFallback(cmdBase, cmdAbsolute, args, options) {
  ...
  const fallback = cmdAbsolute ? cmdBase : null;
  try {
    return await execFileAsync(primary, args, options);
  } catch (err) {
    if (err.code === 'ENOENT' && fallback) {
      return await execFileAsync(fallback, args, options);
    }
    throw err;
  }
}
...
function darwinStrategy(binaryNames) {
  return {
    async findProcess(currentWorkspaceId) {
      const { stdout } = await execWithFallback('ps', '/bin/ps', ['aux'], { encoding: 'utf8', timeout: 5000 });
    ...
    async findListeningPorts(pid) {
      try {
        ...
        return stdout
          .split('\n')
          .map((l) => {
            const m = l.match(/(?:127\.0\.0\.1|\*|localhost|0\.0\.0\.0):(\d+)/);
            return m ? parseInt(m[1]) : null;
          })
          .filter(Boolean);
      } catch {
  ...
  return {
    ...
    async findListeningPorts(pid) {
      try {
        ...
        // ss unavailable or returned empty — fall through to lsof
      }
...
async function _discoverSidecarOnce(ctx) {
  try {
    ...
    const certCandidates = [
      agExt && path.join(agExt.extensionPath, 'dist', 'languageServer', 'cert.pem'),
      '/opt/antigravity-ide/resources/app/extensions/antigravity/dist/languageServer/cert.pem',
      '/usr/share/antigravity/resources/app/extensions/antigravity/dist/languageServer/cert.pem',
