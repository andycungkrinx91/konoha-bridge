# dev-deploy.ps1 — Deploy konoha-bridge source to ALL local Antigravity extension directories
...
if ($PSScriptRoot -and (Test-Path (Join-Path (Split-Path -Parent $PSScriptRoot) "src\extension.js"))) {
    ...
    Write-Error "Could not locate repo root. Run this script from the konoha-bridge directory, or via 'npm run dev:deploy'."
    exit 1
}
