...
describe('getPlatformStrategy', () => {
  it('uses explicit Apple Silicon-aware binary names on darwin', () => {
    ...
    assert.deepEqual(result.binaryNames, [
  ...
  it('keeps the exported platform binary map in sync', () => {
    assert.deepEqual(SIDECAR_BINARY_NAMES.darwin, [
      'language_server_macos_arm',
      'language_server_macos_x64',
      'language_server_macos_arm64',
      'language_server_macos',
    ]);
    assert.deepEqual(SIDECAR_BINARY_NAMES.win32, [
      'language_server_windows_x64.exe',
      'language_server_windows_arm64.exe',
      'language_server_windows.exe',
    ]);
    assert.deepEqual(SIDECAR_BINARY_NAMES.linux, [
...
describe('encodeWorkspaceId', () => {
  it('encodes a Windows path with drive letter (real observed example)', () => {
    // Observed: PID 309976 had --workspace_id file_x_3A_code_marcodiniz_konoha_bridge
    assert.equal(encodeWorkspaceId('x:\\code\\marcodiniz\\konoha-bridge'), 'file_x_3A_code_marcodiniz_konoha_bridge');
  });
