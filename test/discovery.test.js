...
describe('discoverSidecar platform dispatch', () => {
  ...
  it('uses tasklist then wmic then PowerShell process inspection on win32', async () => {
    const harness = loadDiscoveryWithPlatform('win32', {
      'C:\\Windows\\System32\\tasklist.exe': { stdout: '' },
      'C:\\Windows\\System32\\wbem\\wmic.exe': { stdout: '' },
      'C:\\Windows\\System32\\WindowsPowerShell\\v1.0\\powershell.exe': { stdout: '' },
      tasklist: { stdout: '' },
      wmic: { stdout: '' },
    ...
    try {
      const result = await harness.discovery.discoverSidecar(createCtx());
      assert.equal(result, null);
      const executables = harness.calls.map((call) => call.file.split(/[\\/]/).pop().toLowerCase());
      // tasklist is tried first (instant), then wmic full scan, then PowerShell as final fallback per candidate binary
      assert.deepEqual([...new Set(executables)], ['tasklist.exe', 'wmic.exe', 'powershell.exe']);
      assert.ok(!executables.includes('ps'));
    } finally {
      harness.restore();
    }
  });
